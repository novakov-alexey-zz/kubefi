...
pub fn get_files_helper(h: &Helper, hs: &Handlebars, ctx: &Context, _: &mut RenderContext, out: &mut dyn Output) -> HelperResult {
    ...
    if path.exists() {
        ...
        let rendered = hs.render_template(result.as_str(), ctx.data())
...
mod tests {
    ...
    fn print_template() {
        ...
        let template = Template::new(Path::new("./templates"), config).expect("Failed to create template engine");
